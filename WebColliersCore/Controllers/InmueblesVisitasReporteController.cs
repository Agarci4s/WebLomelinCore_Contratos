...
{
    public class InmueblesVisitasReporteController : Controller
    {
        ...
        public ActionResult ExportarExcel(int id, int tipo, int filtro, DateTime fecha, DateTime fecha2, int regimen)
        {
            ...
            using (var libro = new ExcelPackage())
            {
                ...
                if (tipo != 3)
                {
                    ...
                    {
                        worksheet.Cells["X2:X" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "#,##0.00";
                        worksheet.Cells["Y2:Y" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "#,##0.00";
                        worksheet.Cells["Z2:Z" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "#,##0.00";
                        worksheet.Cells["AA2:AA" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "#,##0.00";
                        ...
                        worksheet.Cells["BM2:BM" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "#,##0.00";
                        worksheet.Cells["BN2:BN" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "#,##0.00";

                        worksheet.Cells["BQ2:BQ" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";
                        worksheet.Cells["BR2:BR" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";

                        worksheet.Cells["BS2:BS" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";
                        worksheet.Cells["BT2:BT" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";

                        //
                        worksheet.Cells["BU2:BU" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";
                        worksheet.Cells["BV2:BV" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";
                        worksheet.Cells["BW2:BW" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";

                        worksheet.Cells["BX2:BX" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";
                        worksheet.Cells["BY2:BY" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";
                        worksheet.Cells["BZ2:BZ" + (b_Inmuebles_Visitas.Count + 1)].Style.Numberformat.Format = "$#,##0.00";

                        worksheet.Column(84).Width = 35;
                        worksheet.Column(83).Width = 35;
                        worksheet.Column(80).Width = 35;
                    ...
                    if (tipo != 5)
                    {
                        ...
                        if (tipo==2 && regimen==2)
                        {
                            worksheet.Column(68).Hidden = true;
                            worksheet.Column(69).Hidden = true;
                            worksheet.Column(70).Hidden = true;
                            worksheet.Column(71).Hidden = true;
                        }
                        columnas = columnas - 1;
                    }
